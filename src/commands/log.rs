...
/// Supports filtering by file path, time range, and JSON output.
pub fn log(file: Option<&str>, since: Option<&str>, json: bool) -> Result<()> {
    let root = std::env::current_dir()?;
    let index = Index::open(&root)?;
    let entries = match (file, since) {
        (Some(f), Some(s)) => {
    ...
    if json {
        ...
        for e in &entries {
            ...
            println!("{ts}  {:<8} {hash}  {size:>8}  {}", e.event_type, e.relative_path);
        }
    }
    Ok(())
}
